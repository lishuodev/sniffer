...
namespace WinSniffer
{
            ...
            if (packet is PacketDotNet.EthernetPacket eth)
            {
                ...
                switch (parsed.ethernetType)
                {
                    case EthernetType.IPv4:
                        ...
                        parsed.version = (int)ipv4.Version;
                        parsed.headerLength = ipv4.HeaderLength;
                        parsed.ToS = ipv4.TypeOfService;
                        parsed.totalLength = ipv4.TotalLength;
                        parsed.identification = ipv4.Id;
                        parsed.flags = ipv4.FragmentFlags;
                        parsed.fragmentOffset = ipv4.FragmentOffset;
                        parsed.TTL = ipv4.TimeToLive;
                        parsed.protocol = (byte)ipv4.Protocol;
                        parsed.headerChecksum = ipv4.Checksum;
                        parsed.sourceIP = ipv4.SourceAddress;
                        parsed.destinationIP = ipv4.DestinationAddress;
                        break;
                    case EthernetType.IPv6:
                        var ipv6 = packet.Extract<PacketDotNet.IPv6Packet>();
                        parsed.version6 = (int)ipv6.Version;
                        parsed.trafficClass = ipv6.TrafficClass;
                        parsed.flowLabel = ipv6.FlowLabel;
                        parsed.payloadLength = ipv6.PayloadLength;
                        parsed.nextHeader = (byte)ipv6.NextHeader;
                        parsed.nextHeaderEnum = ipv6.NextHeader;
                        parsed.hopLimit = ipv6.HopLimit;
                        parsed.sourceAddress = ipv6.SourceAddress;
                        parsed.destinationAddress = ipv6.DestinationAddress;
                        break;
    ...
    public class LibraryParsedPacket
    {
        ...
        public PacketDotNet.ProtocolType nextHeaderEnum;
        public int hopLimit;
